...
import { Users2, Plus, RefreshCw, ShieldAlert } from 'lucide-react';
import { Button } from '../components/ui';
import { toast } from 'sonner';
import { toastApiError, extractApiError } from '../utils/apiError';
import type { ChurchCommunity } from '@sfoacc/sdk';

export default function Communities() {
  const client = useSDK();
  const navigate = useNavigate();
  const { selectedUnit } = useAuth();
  const [items, setItems] = useState<ChurchCommunity[]>([]);
  ...
  const load = useCallback((manual = false) => {
    ...
    client.listCommunities({ limit: 100, church_unit_id: selectedUnit?.id })
      ...
      .catch(err => {
        const msg = extractApiError(err, 'Failed to load communities');
        setErrorMsg(msg);
        toastApiError(err, 'Failed to load communities');
      })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-display font-bold text-foreground">Communities</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {items.length > 0 ? `${items.length} communities in ${selectedUnit?.name ?? 'this unit'}` : 'Manage parish communities'}
          </p>
        </div>
      ...
      {errorMsg && (
        <div className="flex items-start gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3.5">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-red-700">Unable to load communities</p>
      ...
      {loading ? (
        ...
        <div className="bg-card border border-border rounded-xl p-12 text-center">
          ...
          <p className="text-sm text-muted-foreground">No communities found for this unit.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map(c => (
            <div key={c.id} className="bg-card border border-border rounded-xl p-5 hover:shadow-md hover:border-[#4cb8d7]/30 transition-all group flex flex-col">
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {(c as unknown as Record<string, unknown>).member_count !== undefined && (
                    <p className="text-[11px] text-muted-foreground mt-0.5">
                      {(c as unknown as Record<string, unknown>).member_count as number ?? 0} members
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 mt-3 pt-3 border-t border-border">
                <Button variant="ghost" size="sm" className="flex-1 justify-center" onClick={() => navigate(`/communities/${c.id}`)}>
                  View
                </Button>
              </div>
            </div>
