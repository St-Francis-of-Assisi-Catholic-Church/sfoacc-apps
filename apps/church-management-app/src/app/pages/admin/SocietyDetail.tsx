...
export default function SocietyDetail({ backPath = '/admin/societies' }: { backPath?: string }) {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (society.status === 'fulfilled') setData((society.value.data as unknown as Data) ?? {});
  ...
  const reloadMembers = () => {
    if (!id) return;
    client.getSocietyMembers(Number(id)).then(r => {
        ...
        <div className="flex items-center gap-3 pb-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Society · ID #{id} · {members.length} member{members.length !== 1 ? 's' : ''}
            </p>
          </div>
          {tab === 'details' && (
            editing ? (
        ...
        {tab === 'members' && (
          <div className="flex-1 min-h-0 mt-5 flex flex-col gap-2">
            ...
            <div className="flex-1 min-h-0 bg-card border border-border rounded-xl overflow-hidden flex flex-col">
              <MembersTable
                members={members}
                avatarClass="bg-[#fef3c7] border-[#f0d070] text-[#9a6c00]"
