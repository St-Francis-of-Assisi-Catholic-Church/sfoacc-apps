...
import type { ParishDetail, OutstationDetail, ChurchUnit, LeadershipRead, MassSchedule, DayOfWeek, MassType } from '@sfoacc/sdk';
  ...
  return (
    <div className="bg-muted/40 border border-border rounded-lg p-4 space-y-3 mb-2">
      ...
      <div className="flex items-center gap-2 pt-1">
        <Button size="sm" isLoading={saving} onClick={onSave}><Save className="w-3.5 h-3.5" /> Save</Button>
        <Button variant="outline" size="sm" onClick={onCancel}><X className="w-3.5 h-3.5" /> Cancel</Button>
      </div>
    </div>
  );
}

type PriestDraft = { name: string; phone: string; email: string };
const EMPTY_PRIEST: PriestDraft = { name: '', phone: '', email: '' };
...
export default function ChurchUnitDetail() {
  ...
  // Collapsible state
  const [expanded, setExpanded] = useState<Set<string>>(new Set(['general', 'contact', 'pastor', 'assistants', 'schedules']));
  ...
  // Assistant priests
  const [assistants, setAssistants] = useState<LeadershipRead[]>([]);
  ...
  // ── Assistant priest actions ──
  const saveAssistantPriest = useCallback(async () => {
    ...
    try {
      if (editingPriestId === 'new') {
        ...
        setAssistants(prev => [...prev, res.data as LeadershipRead]);
        toast.success('Assistant priest added.');
        ...
        setAssistants(prev => prev.map(p => p.id === editingPriestId ? (res.data as LeadershipRead) : p));
  ...
  const deleteAssistantPriest = useCallback(async (priestId: number) => {
    if (!id) return;
    try {
      await client.deleteUnitLeadership(Number(id), priestId);
      setAssistants(prev => prev.filter(p => p.id !== priestId));
    ...
    client.getChurchUnit(Number(id))
      .then(r => {
        ...
        client.listUnitLeadership(Number(id))
          .then(r2 => setAssistants((r2.data as LeadershipRead[] ?? []).filter(l => l.role === 'assistant_priest')))
          .catch(() => undefined);
      })
  ...
  const editSaveCancel = (cardId: string) => ({
    headerAction: editingCard === cardId ? (
      ...
      <Button variant="ghost" size="sm" onClick={() => startEdit(cardId)}><Pencil className="w-3.5 h-3.5" /> Edit</Button>
    ),
  });
    ...
    <div className="space-y-5 pb-10">
      ...
      <div className="sticky top-0 z-20 -mx-6 -mt-6 px-6 pt-5 pb-4 bg-background/95 backdrop-blur-sm border-b border-border/60">
        <div className="flex items-start gap-3">
          <button onClick={() => navigate('/admin/church-units')}
      ...
      {/* ── Pastor ── */}
      <CollapsibleCard
        title="Pastor / Priest in Charge"
        expanded={expanded.has('pastor')} onToggle={() => toggleCard('pastor')}
        {...editSaveCancel('pastor')}
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <Field label="Name" value={d.pastor_name} name="pastor_name" editing={editingCard === 'pastor'} draft={draft} onChange={setField} />
          <Field label="Phone" value={d.pastor_phone} name="pastor_phone" editing={editingCard === 'pastor'} draft={draft} onChange={setField} />
          <div className="sm:col-span-2">
            <Field label="Email" value={d.pastor_email} name="pastor_email" editing={editingCard === 'pastor'} draft={draft} onChange={setField} />
          </div>
        </div>
      </CollapsibleCard>
      ...
      {/* ── Assistant Priests ── */}
      <CollapsibleCard
        title={`Assistant Priests (${assistants.length})`}
        expanded={expanded.has('assistants')} onToggle={() => toggleCard('assistants')}
        headerAction={
          editingPriestId === null ? (
            <Button variant="ghost" size="sm" onClick={() => {
              setPriestDraft(EMPTY_PRIEST);
              setEditingPriestId('new');
              setExpanded(prev => new Set([...prev, 'assistants']));
            }}>
      ...
      >
        <div className="space-y-3">
            ...
            <div key={priest.id}>
              {editingPriestId === priest.id ? (
                <div className="bg-muted/40 border border-border rounded-lg p-4 space-y-3">
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <Button size="sm" isLoading={priestSaving} onClick={saveAssistantPriest}><Save className="w-3.5 h-3.5" /> Save</Button>
                    <Button variant="outline" size="sm" onClick={() => { setEditingPriestId(null); setPriestDraft(EMPTY_PRIEST); }}><X className="w-3.5 h-3.5" /> Cancel</Button>
                  </div>
                </div>
              ) : (
                <div className="flex items-center gap-3 py-2.5 border-b border-border/50 last:border-0">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => deleteAssistantPriest(priest.id)} className="text-destructive hover:text-destructive">
          ...
          {assistants.length === 0 && editingPriestId !== 'new' && (
            <p className="text-sm text-muted-foreground">No assistant priests added yet.</p>
          )}
        </div>
      </CollapsibleCard>
            ...
            <div className="divide-y divide-border">
              {parish.outstations.map(o => (
                <div key={o.id} className="flex items-center gap-3 py-2.5 first:pt-0 last:pb-0">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => navigate(`/admin/church-units/${o.id}`)}>View</Button>
                </div>
              ))}
            </div>
          )}
