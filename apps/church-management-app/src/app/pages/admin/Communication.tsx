...
import {
  ...
  ChevronDown, ChevronUp, Search, Filter, X, Building2, Settings,
...
function TemplateMgmtModal({ templates, onClose, onSave, onDelete }: TemplateMgmtModalProps) {
        ...
        {editing ? (
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <div className="flex gap-2 pt-1">
              <Button onClick={handleSave} isLoading={saving} className="flex-1" title="Save Template">
...
export default function AdminCommunication() {
  ...
  useEffect(() => {
    client.listChurchUnitsPublic()
      .then(r => setChurchUnits((r.data ?? []) as ChurchUnit[]))
      .catch(() => {});
  }, [client]);

  useEffect(() => {
    client.listCommunities({ limit: 100, church_unit_id: selectedUnitId })
      .then(r => setCommunities((r.data?.items ?? []) as Community[]))
      .catch(() => {});
    client.listSocieties({ limit: 100, church_unit_id: selectedUnitId })
      .then(r => setSocieties((r.data?.items ?? []) as Society[]))
      .catch(() => {});
  }, [client, selectedUnitId]);
  ...
  }, [client, selectedUnitId, filterCommunity, filterGender, filterMarital, filterStatus]);
  ...
  const toggleSelectAll = () => {
    ...
    else { setSelectedIds(new Set(filteredRecipients.map(r => r.id))); }
  };
  ...
  const handleSend = async () => {
    ...
    } catch (err) { toastApiError(err, 'Failed to send message'); }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="font-display text-xl font-bold text-foreground">Communication</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Send messages to parishioners via email, SMS, or both</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="bg-card border border-border rounded-xl p-5 space-y-4">
            ...
            {/* Church unit filter */}
            <div>
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">Church Unit</p>
              <div className="relative max-w-sm">
                <Building2 className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground pointer-events-none" />
                <select value={selectedUnitId ?? ''} onChange={e => setSelectedUnitId(e.target.value ? Number(e.target.value) : undefined)}
                ...
                >
                  ...
                  {churchUnits.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                </select>
              </div>
            </div>
                  ...
                  return (
                    <button key={c} onClick={() => setChannel(c)}
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium border transition-all ${
                        channel === c ? 'bg-navy text-white border-navy' : 'bg-background border-border text-foreground hover:bg-muted'
                      }`}
                    >
                  ...
                  {templates.map(t => (
                    <button key={t.id} onClick={() => applyTemplate(t)}
                      className={`w-full text-left px-4 py-3 text-sm hover:bg-muted transition-colors border-b border-border last:border-0 ${
                        selectedTemplateId === t.id ? 'bg-navy/5' : ''
                      }`}
                    >
                      <div className="flex items-center gap-2">
                        <p className="font-medium text-foreground flex-1">{t.name}</p>
                        {selectedTemplateId === t.id && <CheckCircle2 className="w-3.5 h-3.5 text-navy flex-shrink-0" />}
                      </div>
                      {t.description && <p className="text-xs text-muted-foreground mt-0.5">{t.description}</p>}
                    </button>
                  ))}
        ...
        <div className="lg:col-span-2 bg-card border border-border rounded-xl overflow-hidden self-start flex flex-col">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-muted/20">
            ...
            <div className="flex items-center gap-1.5">
              {activeFilterCount > 0 && (
                <button onClick={clearFilters} className="text-[10px] font-medium text-navy hover:text-navy-light flex items-center gap-1">
                  <X className="w-3 h-3" /> Clear
                </button>
              )}
              <button onClick={() => setFiltersOpen(o => !o)}
                className={`flex items-center gap-1.5 text-xs font-medium px-2 py-1 rounded-lg transition-colors ${
                  filtersOpen || activeFilterCount > 0 ? 'bg-navy/10 text-navy' : 'text-muted-foreground hover:text-foreground hover:bg-muted'
                }`}
              >
                <Filter className="w-3 h-3" />
                Filter
                {activeFilterCount > 0 && (
                  <span className="w-4 h-4 bg-navy text-white rounded-full text-[9px] flex items-center justify-center font-bold">
                    {activeFilterCount}
                  </span>
                )}
              </button>
            ...
            <div className="px-3 py-3 border-b border-border bg-muted/20 space-y-2.5">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <select value={filterCommunity} onChange={e => setFilterCommunity(e.target.value)}
                    className="w-full px-2 py-1.5 bg-background border border-input rounded-lg text-xs focus:outline-none"
                  >
              ...
              {loadingSocietyMembers && (
                <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                  <span className="inline-block animate-spin h-2.5 w-2.5 border border-navy border-t-transparent rounded-full" />
              ...
              filteredRecipients.map(r => (
                <label key={r.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-muted/30 cursor-pointer transition-colors">
                  ...
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                    <div className="w-6 h-6 rounded-full bg-navy/10 border border-navy/20 flex items-center justify-center flex-shrink-0">
                      <span className="text-[10px] font-semibold text-navy">{r.first_name?.charAt(0) ?? '?'}</span>
                    </div>
